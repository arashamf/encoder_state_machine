...
void OLED_SendCommand(uint8_t command)
{
	...
	LL_SPI_TransmitData8(OLED_SPI, command);
	LCD_CS(OFF);
}
...
uint8_t OLED_init(void)
{
	...
	OLED_Invert(NONE_INVERTED); //инвертированный экран - белый фон
...
void OLED_DrawBitmap(const uint8_t *progmem_bitmap, uint8_t height, uint8_t width, uint8_t pos_x, uint8_t pos_y)
{
	...
	for (uint8_t current_y = 0; current_y < height; current_y++)
	{
		for (uint8_t current_x = 0; current_x < width; current_x++)
		{
			...
			{	OLED_DrawPixelStatus(current_x + pos_x, current_y + pos_y, 1);	}										OLED_DrawPixelStatus(current_x + pos_x, current_y + pos_y, 0);
		}
	}
}
